...
(define (map f lst)
  (define (loop lst acc)
    (if (empty? lst)
        (reverse acc)
        (loop (rest lst) (cons (f (first lst)) acc))))
  (loop lst '()))
...
(fold-left - 1000 (list 10 20 40))
 = (fold-left - (- 1000 10) '(20 40))
 = (fold-left - (- (- 1000 10) 20) '(40))
 = (fold-left - (-
                  (-
                    (- 1000 10)
...
(define-test-suite test-fold-left
  ...
  (test-equal? "soma é associativa"
               ...
               15)
  (test-equal? "teste do exemplo" (fold-left - 1000 (list 10 20 40)) 930)
...
(define (pontuacao-final-fr ini lstded)
  (fold-right (lambda (a b) (- b a)) ini (reverse lstded)))
...
(define (concat-lista lista-str)
  (fold-left string-append "" lista-str))
