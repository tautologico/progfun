...
(define (filtra-par-lg-2 lst)
  (filter (lambda (x) (if (integer? x)
                          (even? x)
                          #f))
          lst))
...
(define (tamanho-lista-strings-2 lstr)
  (string-length (foldl string-append "" lstr)))
