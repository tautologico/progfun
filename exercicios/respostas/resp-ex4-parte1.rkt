...
(define (soma-acc m n acc)
  (if (zero? m)
      acc
      (soma-acc (sub1 m) n (add1 acc))))

(define (soma-tr m n)
  (if (zero? m)
      n
      (soma-tr (sub1 m) (add1 n))))
...
(define (par-acc n acc)
  (if (zero? n)
      acc
      (par-acc (sub1 n) (not acc))))
...
(define elem3-lista6-2 (first (rest (rest lista6))))
(define elem3-lista6 (caddr lista6))
...
(define (elemento-n lista n)
  (cond [(empty? lista)  #f]
        [(zero? n)       (first lista)]
        [else            (elemento-n (rest lista) (sub1 n))]))
...
(define (quadrado-acc-1 l acc)
  (if (empty? l)
      acc
      (quadrado-acc-1 (rest l) (append acc (list (quadrado (first l)))))))

(define (quadrado-acc-2 l acc)
  (if (empty? l)
      (reverte-lista acc)
      (quadrado-acc-2 (rest l) (cons (quadrado (first l))
                                     acc))))
...
(quadrado-lista '(1 2 3))
 = (loop '(1 2 3) '())
 = (loop '(2 3) (cons (quadrado 1) '()))
 = (loop '(2 3) '(1))
 = (loop '(3) (cons (quadrado 2) '(1)))
 = (loop '(3) '(4 1))
 = (loop '()  (cons (quadrado 3) '(4 1)))
 = (loop '()  '(9 4 1))
 = '(9 4 1)
...
(define (dobra-lista l)
  (define (loop l acc)
    (if (empty? l)
        (reverte-lista acc)
        (loop (rest l) (cons (* 2 (first l)) acc))))
  (loop l '()))
...
(define (filtra-par l)
  (define (loop l acc)
    (cond [(empty? l) (reverte-lista acc)]
          [(par (first l)) (loop (rest l) (cons (first l) acc))]
